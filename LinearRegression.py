...
def evaluate_model_on_dataframe(model, model_name):
    ...
    top5_percent = top5_hits / (len(unique_dates) * 2) * 100
    top10_percent = top10_hits / (len(unique_dates) * 5) * 100

    # Print the results
    print(f"{model_name} - Top 1: {top1_percent:.2f}% of entries got a hit")
    print(f"{model_name} - Top 2: {top5_percent:.2f}% of entries got a hit")
    print(f"{model_name} - Top 5: {top10_percent:.2f}% of entries got a hit")
